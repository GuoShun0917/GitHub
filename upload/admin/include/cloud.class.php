...
class Cloud {
    ...
    function install($type, $cloud_id, $mode) {
        ...
        if ($type == 'module') {
            ...
            if ($module_type) { // 如果不存在模块类型，则不写入系统文件
                if (!$this->change_system_php($cloud_id, $module_type))
        ...
        $GLOBALS['dou']->dou_clear_cache(ROOT_PATH . 'cache'); // 更新缓存
     ...
     * +----------------------------------------------------------
     */
    function change_system_php($cloud_id, $type = '', $del = false) {
        // 读取模块列表
    ...
    function change_nav($cloud_id, $module_type = '', $del = false) {
        if ($del) {
            $GLOBALS['dou']->delete($GLOBALS['dou']->table('nav'), "module = '$cloud_id'");
            $GLOBALS['dou']->delete($GLOBALS['dou']->table('nav'), "module = '$cloud_id" . '_category' . "'");
        } else {
    ...
    function clear_module($cloud_id) {
        ...
        // STEP3 修改系统文件-删除操作
        $this->change_system_php($cloud_id, '', true);
    }
